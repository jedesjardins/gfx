...
private:
    ...
    //
    friend void deleteUniforms(void const * data);
    friend struct DeleteUniforms;

    std::unique_ptr<Device>              device;
    std::unique_ptr<FrameResources>      frames;
    std::unique_ptr<ImageResources>      images;
    std::unique_ptr<RenderPassResources> render_passes;
    std::unique_ptr<UniformResources>    uniforms;
    std::unique_ptr<PipelineResources>   pipelines;
    std::unique_ptr<CommandResources>    commands;
    std::unique_ptr<BufferResources>     buffers;
...
{
public:
    Device(GLFWwindow * window_ptr): window{window_ptr}
    {}
...
struct Renderer::FrameResources
{
public:
...
private:
                                ...
                                TextureHandle &          attachment)
    {
        ...
        if (attachment_config.format == Format::USE_COLOR)
        {
            ...
            // final_layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL; // subpass dependencies
            // handle this
        }
        else if (attachment_config.format == Format::USE_DEPTH)
        {
            ...
            // final_layout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL; // subpass
...
struct Renderer::RenderPassResources
{
public:
    bool init(RenderConfig &         render_config,
              Device const *         device,
    ...
    ErrorCode createRenderPasses(Device const * device, ImageResources const * image_resources)
    {
        for (size_t rp_i = 0; rp_i < render_passes.size(); ++rp_i)
        {
            ...
            for (auto & iter: render_pass_config.descriptions)
            {
                ...
                auto opt_attachment_handle = image_resources->get_attachment_handle(
                    attachment_name);
            ...
            for (auto & subpass_info: render_pass_config.subpasses)
            {
                ...
                if (subpass_info.preserve_attachments.size() > 0)
                {
                    subpasses.back().preserveAttachmentCount
                        = subpass_info.preserve_attachments.size();
                    subpasses.back().pPreserveAttachments
                        = subpass_info.preserve_attachments.data();
                }
            ...
            VK_CHECK_RESULT(
                vkCreateRenderPass(
                    device->get_logical_device(), &renderPassInfo, nullptr, &render_pass),
                "Unable to create VkRenderPass");

            auto error = createFramebuffer(device,
                                           image_resources,
                                           render_pass_config,
                                           render_pass,
                                           framebuffers[rp_i],
                                ...
                                VkExtent2D &                 extent)
    {
        ...
        for (size_t i = 0; i < device->get_image_count(); ++i)
        {
            ...
            for (auto iter: config.attachments)
            {
                ...
                auto attachment_handle
                    = image_resources->get_attachment_handle(attachment_name).value();
                auto opt_attachment_config = image_resources->get_config(attachment_handle);
                ...
                else
                {
                    ...
                    auto opt_attachment_handle = image_resources->get_texture_handle(
                        attachment_handle);
...
class Renderer::BufferResources
{
public:
    bool init()
    {
                                              ...
                                              VkMemoryPropertyFlags properties)
    {
        ...
        if (buffer.create(device->get_physical_device(),
                          device->get_logical_device(),
                          size,
                          usage,
                          properties)
...
class Renderer::UniformResources
{
public:
    bool init(RenderConfig & render_config, Device const * device)
    {
    ...
    void update_uniform(Device const *          device,
                        BufferResources const * buffers,
                        ...
                        UniformWrite *          uniform_write_infos)
    {
        ...
        vkUpdateDescriptorSets(
            device->get_logical_device(), writes.size(), writes.data(), 0, nullptr);
    }
    ...
    std::optional<VkDescriptorSetLayout> get_layout(std::string const & name) const
    {
...
private:
                                    ...
                                    std::vector<VkDescriptorSetLayoutBinding> const & bindings)
    {
        ...
        VK_CHECK_RESULT(vkCreateDescriptorSetLayout(
                            device->get_logical_device(), &layoutInfo, nullptr, &layout),
...
struct Renderer::PipelineResources
{
public:
    bool init(RenderConfig &           render_config,
              Device const *           device,
              ...
              UniformResources const * uniforms)
    {
        ...
        for (auto & iter: render_config.push_constants)
        {
            ...
            LOG_DEBUG(
                "Added Push Constant Handle {} for Binding {}", push_constant_handle, iter.first);
        }
...
private:
    void destroy_pipelines(Device const * device)
    {
        for (auto & pipeline: pipelines)
        {
            ...
            vkDestroyPipelineLayout(
                device->get_logical_device(), pipeline.vk_pipeline_layout, nullptr);
        }
    }
                              ...
                              PipelineConfig const &   pipeline_config)
    {
        ...
        render_passes->per_render_pass_subpass_pipelines[render_pass_handle][subpass_handle]
...
struct Renderer::CommandResources
{
public:
    bool init(RenderConfig & render_config, Device const * device)
    {
...
struct DeleteUniforms
{
    ...
    Renderer::UniformResources * uniform_resources;
...
Renderer::Renderer(GLFWwindow * window_ptr)
: device{std::make_unique<Renderer::Device>(window_ptr)},
  frames{std::make_unique<Renderer::FrameResources>()},
  images{std::make_unique<Renderer::ImageResources>()},
  render_passes{std::make_unique<Renderer::RenderPassResources>()},
  uniforms{std::make_unique<Renderer::UniformResources>()},
  pipelines{std::make_unique<Renderer::PipelineResources>()},
  commands{std::make_unique<Renderer::CommandResources>()},
  buffers{std::make_unique<Renderer::BufferResources>()}
{}
...
bool Renderer::init(RenderConfig & render_config)
{
    ...
    if (!images->init(render_config, device.get()))
    {
        LOG_ERROR("Failed to initialize ImageResources in Renderer");
        return false;
    }

    if (!render_passes->init(render_config, device.get(), images.get()))
    {
    ...
    if (!uniforms->init(render_config, device.get()))
    {
        LOG_ERROR("Failed to initialize UniformResources in Renderer");
        return false;
    }

    if (!pipelines->init(render_config, device.get(), render_passes.get(), uniforms.get()))
    {
        LOG_ERROR("Failed to initialize PipelineResources in Renderer");
        return false;
    }

    if (!commands->init(render_config, device.get()))
    {
...
void Renderer::quit()
{
    ...
    buffers->quit(device.get());
    render_passes->quit(device.get());
    images->quit(device.get());
...
void Renderer::delete_uniforms(size_t uniform_count, UniformHandle const * uniform_handles)
{
    ...
    delete_command->uniform_resources = uniforms.get();
